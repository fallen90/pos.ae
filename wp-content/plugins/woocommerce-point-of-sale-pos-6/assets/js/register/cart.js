	...
	window.POS_CART = {
		...
		addToCart : function (product_data, product_id, quantity, variation_id, variation, cart_item_data) {
			...
			try{
				...
				/*if ( did_action( 'wp' ) ) {
					CART.set_cart_cookies( ! CART.is_empty() );
				}*/
		...
		generate_cart_id : function (product_id, variation_id, variation, cart_item_data) {
			...
			if ( typeof cart_item_data == 'object' && cart_item_data.length > 0 ) {
				...
				$.each(cart_item_data, function(key, value) {
					...
					if ( typeof value == 'object' ) {
						value = _http_build_query_helper( value );
					}
		...
		*/
		...
		calculate_totals : function () {
			...
			if(CART.prices_include_tax && CART.subtotal_ex_tax > 0 && wc_pos_params.pos_calc_taxes === true ){
				var subtotal_amount_row = accountingPOS(CART.subtotal_ex_tax, 'formatMoney') + '<small>' + pos_cart.ex_tax_or_vat + '</small>';
			}else{
			...
			if ( Object.size(CART.coupons) > 0 ) {
				...
				$.each(CART.coupons, function(coupon_code, the_coupon) {
					...
					var template = Handlebars.compile(source);
					var context  = {'coupon_code': coupon_code, 'amount': value};
					var html     = template(context);					
		 ...
		 */
		add_custom_discount : function ( amount, type ) {
			if( typeof amount == 'undefined' || amount <= 0 ) return false;
			if( typeof type == 'undefined' ){
				type = 'fixed_cart';
			}

			var coupon_code = 'POS Discount';
			...
			the_coupon.add_coupon_message( WC_POS_DISCOUNT_SUCCESS );
			CART.calculate_totals();
		},
		...
		add_discount : function ( coupon_code ) {
			...
			var q = APP.db.from('coupons');
			q = q.where('code', '=', coupon_code);
			q.list(1).done( function( record ) {

...
jQuery(document).ready(function($) { 
	...
	APP       = window.POS_APP = {
		...
		init : function () {
            ...
            APP.db = new ydn.db.Storage( 'WC-POS', APP.schema );        
        ...
        sync_data : function(){
            ...
            $.when(APP.getServerCustomersCount()).then(function(CustomersCount) {
              if(parseInt(CustomersCount.count) > 0){
                ...
                APP.setCookie('pos_LastOffsetCoupon',APP.lastOffsetCustomer, 5);
              }
            });            
        },
        				...
        				if( pos_custom_product.id == item.product_id || item.product_id == null ){
                            var adding_to_cart = clone(pos_custom_product);
                                ...
                                if( item.tax_class != null){
                                    adding_to_cart.tax_class  = item.tax_class;
                            ...
                            APP.addToCart(item.product_id, quantity, variation_id, variation, 0, item.id );                            
                        }
        ...
        addToCart : function (product_id, quantity, variation_id, variation, cart_item_data, item_id) {
...
*/
	    	...
	    	if( pos_grid.second_column_layout == 'product_grids'){
	    		...
	    		if( pos_grid.grid_id == 'categories' ){
	    			$.each(pos_grid.categories, function(i, cat) {
	    				var $li = $('<li id="category_'+i+'" class="title_category open_category category_cycle" data-catid="'+i+'"><span></span></li>');
	    				$li.find('span').text(cat.name);
	    ...
	    ready: function () {           
            ...
            if(pos_default_customer){
            	CUSTOMER.set_default_data(pos_default_customer);
            }
            ...
            $('#save_customer').on('click', function() {
            	...
            	if(err == 0){
                    ...
                    $.each(additional_fields, function(index, key) {
                        var a_el = $('#customer_details .col-3 #'+key);
                        if(a_el.length){
			...
			$('#modal-order_payment').on('click', 'input.go_payment', function(){
                ...
                if( email_receipt && CUSTOMER.billing_address.email == '' ){
                    args = {
                        ...
                        confirm : function(answer){
                            if( answer && answer != ''){
                                CUSTOMER.billing_address.email = answer;
                            }
                            APP.createOrder(true);
                        }
                    };
    ...
    changeProductPrice = function (el) {
        ...
        if( typeof CART.cart_contents[ cart_item_key ] != 'undefined'){
            ...
            CART.calculate_totals();
            //console.log(CART.cart_contents[ cart_item_key ].original_price);
            //change_price_timer = setTimeout(function(){ CART.calculate_totals(); }, 500);            
        }
    }

...
class WC_POS_Admin_Setup_Wizard {
	...
	public function wc_pos_setup_general_options() {		
		// Defaults
		$country_setting = get_option( 'wc_pos_default_country', 'GB' );
				...
				<tbody>
					...
					?><tr valign="top">
						<th scope="row" class="titledesc">
							<?php _e( 'Default Country', 'woocommerce' ); ?>
						</th>
						<td>
							<select name="wc_pos_default_country" data-placeholder="<?php esc_attr_e( 'Choose a country&hellip;', 'woocommerce' ); ?>" title="<?php esc_attr_e( 'Country', 'woocommerce' ) ?>" class="wc-enhanced-select">
					...
					wc_pos_select( array( 
								...
								'label'             =>  __( 'Load Order ', 'woocommerce' ) ,
								'class'             => 'wc-enhanced-select',
								'type'              => 'multiselect',
	...
	public function exist_oulet()
	{
		global $wpdb;
		$result = false;
		$table  = $wpdb->prefix . 'wp_wc_poin_of_sale_outlets';

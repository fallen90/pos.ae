						...
						if( isset($item['id']) && $item['id'] > 0){
							if ( $item['product_id'] > 0 && $item['product_id'] != $custom_pr_id) {
								...
								if ( $_product && $_product->exists() && $_product->managing_stock() ) {
									$qty       = (int)$item_meta['_qty'][0];
									
									$new_stock = $_product->increase_stock( $qty );

									/*if ( isset( $item['variation_id'] ) && $item['variation_id'] ) {
										$order->add_order_note( sprintf( __( 'POS: Item #%s variation #%s stock reduced from %s to %s.', 'woocommerce' ), $item['product_id'], $item['variation_id'], $new_stock - $qty, $new_stock) );
									} else {
										$order->add_order_note( sprintf( __( 'POS: Item #%s stock reduced from %s to %s.', 'woocommerce' ), $item['product_id'], $new_stock - $qty, $new_stock) );
									}
									$order->send_stock_notifications( $_product, $new_stock, $item['quantity'] );*/
								}
							}
							$this->set_line_item( $order, $item, 'update' );							
						}else{
							$this->set_line_item( $order, $item, 'create' );							
						}

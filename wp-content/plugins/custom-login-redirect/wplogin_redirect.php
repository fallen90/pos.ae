...
Version: 2.9.0
Text Domain: custom-login-redirect
Change Log:
...
{
	load_plugin_textdomain( 'custom-login-redirect', PLUGINDIR . '/' . dirname( plugin_basename(__FILE__) ), dirname( plugin_basename(__FILE__) ) );
}
...
class rulRedirectFunctionCollection
{
    ...
    */
    static function get_settings( $setting=false )
    {
        ...
        // Then, set that plugin to redirect to http://www.yoursite.com/wp-content/plugins/custom-login-redirect/wplogin_redirect_control.php
...
function rul_register( $before = '<li>', $after = '</li>', $give_echo = true ) {
	...
	if ( ! is_user_logged_in() ) {
		if ( get_option('users_can_register') )
			$link = $before . '<a href="' . site_url('wp-login.php?action=register', 'login') . '">' . __('Register', 'custom-login-redirect') . '</a>' . $after;
		else
			$link = '';
	} else {
        $link = $before . '<a href="' . redirect_to_front_page('', '', $current_user) . '">' . __('Site Admin', 'custom-login-redirect') . '</a>' . $after;;
	}
...
if( is_admin() )
{
    ...
    function rul_validate_submission( $typeValue, $type )
    {
        ...
        if( $type == 'user' )
        {
            ...
            {
                $success = false;
                $error_message = '<p><strong>****' .__('ERROR: Non-existent username submitted ','custom-login-redirect') .'****</strong></p>';
            }
        }
        elseif( $type == 'role' )
        {
            ...
            {
                $success = false;
                $error_message = '<p><strong>****' .__('ERROR: Non-existent role submitted ','custom-login-redirect') .'****</strong></p>';
            }
        }
        elseif( $type == 'level' )
        {
            ...
            {
                $success = false;
                $error_message = '<p><strong>****' .__('ERROR: Non-existent level submitted ','custom-login-redirect') .'****</strong></p>';
            }
        }
    ...
    function rul_validate_deletion( $typeValue, $type )
    {
        ...
        {
            $success = false;
            $error_message = '<p><strong>****' . sprintf( __('ERROR: Empty %s submitted ','custom-login-redirect' ), $type ) . '****</strong></p>';
        }
        return array( 'success' => $success, 'error_message' => $error_message );
    }
        ...
        if( $typeValue && ( $address || $address_logout ) )
        {
            ...
            if( $rul_submit_success )
            {
                ...
                if( !$address && !$address_logout )
                {
                    ...
                    $rul_process_submit = '<p><strong>****' . sprintf( __( 'ERROR: Non-local or invalid URL submitted for %s %s','custom-login-redirect' ), $type, $typeValue ) . '****</strong></p>';
                }
                else
                {
                    // Insert a new rule
                    ...
                    {
                        $rul_submit_success = false;
                        $rul_process_submit = '<p><strong>****' . sprintf( __('ERROR: Unknown error adding %s-specific redirect for %s %s','custom-login-redirect' ), $type, $type, $typeValue ) . '****</strong></p>';
                    }
                }
            }
            ...
            if( $rul_submit_success )
            {
                $rul_process_submit = '<p>' . sprintf( __( 'Successfully added %s-specific redirect rule for %s', 'custom-login-redirect' ), $type, $typeValue ) . '</p>';
            }
        }
        ...
        if( $typeValue && ( $address || $address_logout ) )
        {
            ...
            if( $rul_submit_success )
            {
                ...
                else
                {
                    ...
                    {
                        $rul_submit_success = false;
                        $rul_process_submit = '<p><strong>****' . sprintf( __('ERROR: Unknown error editing %s-specific redirect for %s %s','custom-login-redirect' ), $type, $type, $typeValue ) . '****</strong></p>';
                    }
                }
            }
            ...
            if( $rul_submit_success )
            {
                $rul_process_submit = '<p>' . sprintf( __( 'Successfully edited %s-specific redirect rule for %s', 'custom-login-redirect' ), $type, $typeValue ) . '</p>';
            }
        }
    ...
    function rul_delete_rule( $typeValue, $type )
    {
        ...
        if( $typeValue )
        {
            ...
            if( $rul_submit_success )
            {
                ...
                {
                    $rul_submit_success = false;
                    $rul_process_submit = '<p><strong>****' . sprintf( __('ERROR: Unknown error deleting %s-specific redirect for %s %s','custom-login-redirect' ), $type, $type, $typeValue ) . '****</strong></p>';
                }
            }

            if( $rul_submit_success )
            {
                $rul_process_submit = '<p>' . sprintf( __( 'Successfully deleted %s-specific redirect rule for %s', 'custom-login-redirect' ), $type, $typeValue ) . '</p>';
            }
        }
    ...
    function rul_submit_all( $update_or_delete, $address, $address_logout )
    {
        ...
        if( $update_or_delete == 'delete' )
        {
            ...
            if( $update === false )
            {
                $rul_process_submit .= '<p><strong>****' .__('ERROR: Unknown database problem removing URL for &#34;all other users&#34; ','custom-login-redirect') .'****</strong></p>';
            }
            else
            {
                $rul_process_submit .= '<p>'.__('Successfully removed URL for &#34;all other users&#34; ','custom-login-redirect') .'</p>';
            }
        }
        ...
        elseif( $update_or_delete == 'update' )
        {
            ...
            {
                $rul_process_submit .= '<p><strong>****' .__('ERROR: Non-local or invalid URL submitted ','custom-login-redirect') .'****</strong></p>';
            }
            ...
            else
            {
                ...
                if( $update === false )
                {
                    $rul_process_submit .= '<p><strong>****' .__('ERROR: Unknown database problem updating URL for &#34;all other users&#34; ','custom-login-redirect') .'****</strong></p>';
                }
                else
                {
                    $rul_process_submit .= '<p>'.__('Successfully updated URL for &#34;all other users&#34;','custom-login-redirect') .'</p>';
                }
            }
        }
    ...
    function rul_submit_register( $update_or_delete, $address )
    {
        ...
        if( $update_or_delete == 'delete' )
        {
            ...
            if ( $update === false )
            {
                $rul_process_submit .= '<p><strong>****' . __( 'ERROR: Unknown database problem removing URL for &#34;post-registration&#34; ','custom-login-redirect') .'****</strong></p>';
            }
            else {
                $rul_process_submit .= '<p>' . __( 'Successfully removed URL for &#34;post-registration&#34; ', 'custom-login-redirect' ) .'</p>';
            }
        }
        ...
        elseif( $update_or_delete == 'update' )
        {
            ...
            else
            {
                ...
                if( $update === false )
                {
                    $rul_process_submit .= '<p><strong>****' .__('ERROR: Unknown database problem updating URL for &#34;post-registration&#34; ','custom-login-redirect') .'****</strong></p>';
                }
                else
                {
                    $rul_process_submit .= '<p>'.__('Successfully updated URL for &#34;post-registration&#34;','custom-login-redirect') .'</p>';
                }
            }
        }
    ...
    function rul_submit_settings()
    {
        ...
        $rul_process_submit = '<div id="message" class="updated fade">';
        $rul_process_submit .= '<p>' . __( 'Successfully updated plugin settings', 'custom-login-redirect' ) . '</p>';
        ...
        if( $rul_rules )
        {
            ...
            while( $i < count( $rul_rules ) )
            {
                ...
                if( $rul_type == 'user' )
                {
                    ...
                    $rul_usernamevalues .= '<p>' . __('Login URL', 'custom-login-redirect' ) . '<br /><input type="text" size="90" maxlength="500" name="rul_username_address" value="' . $rul_url . '" /></p>';
                    $rul_usernamevalues .= '<p>' . __('Logout URL', 'custom-login-redirect' ) . '<br /><input type="text" size="60" maxlength="500" name="rul_username_logout" value="' . $rul_url_logout . '" /></p>';
                    $rul_usernamevalues .= '</td>';
                    $rul_usernamevalues .= '<td><p><input name="rul_username_edit" type="submit" value="' . __( 'Edit', 'custom-login-redirect' ) . '" /> <input type="submit" name="rul_username_delete" value="' . __( 'Delete', 'custom-login-redirect' ) . '" /></p></td>';
                    $rul_usernamevalues .= '</tr>';
                ...
                elseif( $rul_type == 'role' )
                {
                    ...
                    $rul_rolevalues .= '<td>';
                    $rul_rolevalues .= '<p>' . __('Login URL', 'custom-login-redirect' ) . '<br /><input type="text" size="90" maxlength="500" name="rul_role_address" value="' . $rul_url . '" /></p>';
                    $rul_rolevalues .= '<p>' . __('Logout URL', 'custom-login-redirect' ) . '<br /><input type="text" size="60" maxlength="500" name="rul_role_logout" value="' . $rul_url_logout . '" /></p>';
                    $rul_rolevalues .= '</td>';
                    $rul_rolevalues .= '<td><p><input name="rul_role_edit" type="submit" value="' . __( 'Edit', 'custom-login-redirect' ) . '" /> <input type="submit" name="rul_role_delete" value="' . __( 'Delete', 'custom-login-redirect' ) . '" /></p></td>';
                    $rul_rolevalues .= '</tr>';
                    $rul_rolevalues .= '</form>';
                ...
                elseif( $rul_type == 'level' )
                {
                    ...
                    $rul_levelvalues .= '<td>';
                    $rul_levelvalues .= '<p>' . __('Login URL', 'custom-login-redirect' ) . '<br /><input type="text" size="90" maxlength="500" name="rul_level_address" value="' . $rul_url . '" /></p>';
                    $rul_levelvalues .= '<p>' . __('Logout URL', 'custom-login-redirect' ) . '<br /><input type="text" size="60" maxlength="500" name="rul_level_logout" value="' . $rul_url_logout . '" /></p>';
                    $rul_levelvalues .= '</td>';
                    $rul_levelvalues .= '<td><p><input name="rul_level_order" type="text" size="2" maxlength="2" value="' . $rul_order . '" /></td>';
                    $rul_levelvalues .= '<td><p><input name="rul_level_edit" type="submit" value="' . __( 'Edit', 'custom-login-redirect' ) . '" /> <input type="submit" name="rul_level_delete" value="' . __( 'Delete', 'custom-login-redirect' ) . '" /></p></td>';
                    $rul_levelvalues .= '</tr>';
...
?>
    <div class="wrap">
        <h2><?php _e('Manage redirect rules', 'custom-login-redirect' ); ?></h2>
        <?php print $rul_process_submit; ?>
        <p><?php _e('Define custom URLs to which different users, users with specific roles, users with specific levels, and all other users will be redirected upon login.', 'custom-login-redirect' ); ?></p>
        <p><?php _e('Define a custom URL to which all users will be redirected upon logout', 'custom-login-redirect' ); ?></p>
        <p><?php _e('Note that you can use the syntax <strong>[variable]username[/variable]</strong> in your URLs so that the system will build a dynamic URL upon each login, replacing that text with the users username.', 'custom-login-redirect' ); ?></p>

        <h3><?php _e('Specific users', 'custom-login-redirect' ); ?></h3>
        <?php
        ...
        <form name="rul_username_add_form" action="<?php print '?page=' . basename(__FILE__); ?>" method="post">
            <p><?php _e('Add:', 'custom-login-redirect' ); ?> 
                <select name="rul_username" >
                    <option value="-1"><?php _e('Select a username', 'custom-login-redirect' ); ?></option>
                    <?php print rul_returnusernames($rul_usernames_existing); ?>
                </select>
                <br /><?php _e('URL:', 'custom-login-redirect' ); ?> <input type="text" size="90" maxlength="500" name="rul_username_address" />
                <br /><?php _e('Logout URL:', 'custom-login-redirect' ); ?> <input type="text" size="90" maxlength="500" name="rul_username_logout" />
            </p>
            <p class="submit"><input type="submit" name="rul_username_submit" value="<?php _e('Add username rule', 'custom-login-redirect' ); ?>" /></p>
        </form>
            
        <h3><?php _e('Specific roles', 'custom-login-redirect' ); ?></h3>
        <?php
            if( $rul_rolevalues )
            {
                print '<table class="widefat">';
        ...
        <form name="rul_role_add_form" action="<?php print '?page=' . basename(__FILE__); ?>" method="post">
            <p><?php _e('Add:', 'custom-login-redirect' ); ?> 
                <select name="rul_role" >
                    <option value="-1"><?php _e('Select a role', 'custom-login-redirect' ); ?></option>
                    <?php print rul_returnroleoptions($rul_roles_existing); ?>
                </select>
                <br /><?php _e('URL:', 'custom-login-redirect' ); ?>  <input type="text" size="90" maxlength="500" name="rul_role_address" />
                <br /><?php _e('Logout URL:', 'custom-login-redirect' ); ?>  <input type="text" size="90" maxlength="500" name="rul_role_logout" />
            </p>
            <p class="submit"><input type="submit" name="rul_role_submit" value="<?php _e( 'Add role rule', 'custom-login-redirect' ); ?>" /></p>
        </form> 
 
        <h3><?php _e('Specific levels', 'custom-login-redirect' ); ?></h3>
        <?php
            if( $rul_levelvalues )
            {
                print '<table class="widefat">';
        ?>
                <tr>
                    <th></th>
                    <th></th>
                    <th><?php _e('Order', 'custom-login-redirect' ); ?></th>
                    <th></th>
                </tr>
        <?php
        ...
        <form name="rul_level_add_form" action="<?php print '?page=' . basename(__FILE__); ?>" method="post">
            <p><?php _e('Add:', 'custom-login-redirect' ); ?> 
                <select name="rul_level" >
                    <option value="-1"><?php _e('Select a level', 'custom-login-redirect' ); ?></option>
                    <?php print rul_returnleveloptions($rul_levels_existing); ?>
                </select>
                <br /><?php _e('Order:', 'custom-login-redirect' ); ?> <input type="text" size="2" maxlength="2" name="rul_level_order" />
                <br /><?php _e('URL:', 'custom-login-redirect' ); ?> <input type="text" size="90" maxlength="500" name="rul_level_address" />
                <br /><?php _e('Logout URL:', 'custom-login-redirect' ); ?> <input type="text" size="90" maxlength="500" name="rul_level_logout" />
            </p>
            <p class="submit"><input type="submit" name="rul_level_submit" value="<?php _e('Add level rule', 'custom-login-redirect' ); ?>" /></p>
        </form> 
        
        <h3><?php _e( 'All other users', 'custom-login-redirect' ); ?></h3>
        <form name="rul_allform" action="<?php '?page=' . basename(__FILE__); ?>" method="post">
            <p><?php _e('URL:', 'custom-login-redirect' ) ?> <input type="text" size="90" maxlength="500" name="rul_all" value="<?php print $rul_allvalue; ?>" /></p>
            <p><?php _e('Logout URL:', 'custom-login-redirect' ) ?> <input type="text" size="90" maxlength="500" name="rul_all_logout" value="<?php print $rul_allvalue_logout; ?>" /></p>
            <p class="submit"><input type="submit" name="rul_allupdatesubmit" value="<?php _e('Update', 'custom-login-redirect' ); ?>" /> <input type="submit" name="rul_alldeletesubmit" value="<?php _e('Delete', 'custom-login-redirect' ); ?>" /></p>
        </form>
        
        <hr />
        
        <h3><?php _e( 'Post-registration', 'custom-login-redirect' ); ?></h3>
        <form name="rul_registerform" action="<?php '?page=' . basename(__FILE__); ?>" method="post">
            <p><?php _e( 'URL:', 'custom-login-redirect' ) ?> <input type="text" size="90" maxlength="500" name="rul_register" value="<?php print $rul_registervalue; ?>" /></p>
            <p class="submit"><input type="submit" name="rul_registerupdatesubmit" value="<?php _e( 'Update', 'custom-login-redirect' ); ?>" /> <input type="submit" name="rul_registerdeletesubmit" value="<?php _e( 'Delete', 'custom-login-redirect' ); ?>" /></p>
        </form>
        
        <hr />
        
        <h3><?php _e( 'Customize plugin settings', 'custom-login-redirect' ); ?></h3>
        <form name="rul_settingsform" action="<?php print '?page=' . basename(__FILE__); ?>" method="post">
        <table class="widefat">
        <tr>
            <td>
                <p><strong><?php _e( 'Redirect restrictions', 'custom-login-redirect' ); ?></strong></p>
            </td>
            <td>
                <select name="rul_local_only">
                    <option value="1"<?php if( 1 == $rul_settings['rul_local_only'] ) print ' selected="selected"'; ?>><?php _e( 'Any http or https URL', 'custom-login-redirect' ); ?></option>
                    <option value="2"<?php if( 2 == $rul_settings['rul_local_only'] ) print ' selected="selected"'; ?>><?php _e( 'Any URL', 'custom-login-redirect' ); ?></option>
                    <option value="3"<?php if( 3 == $rul_settings['rul_local_only'] ) print ' selected="selected"'; ?>><?php _e( 'Any URL on the same domain', 'custom-login-redirect' ); ?></option>
                </select>
            </td>
        </tr>
        <tr>
            <td>
                <p><strong><?php _e( 'Allow a POST or GET &#34;redirect_to&#34; variable to take redirect precedence', 'custom-login-redirect' ); ?></strong></p>
            </td>
            <td>
                <select name="rul_allow_post_redirect_override">
                    <option value="1"<?php if( $rul_settings['rul_allow_post_redirect_override'] ) print ' selected="selected"'; ?>><?php _e( 'Yes', 'custom-login-redirect' ); ?></option>
                    <option value="0"<?php if( !$rul_settings['rul_allow_post_redirect_override'] ) print ' selected="selected"'; ?>><?php _e( 'No', 'custom-login-redirect' ); ?></option>
                </select>
            </td>
        </tr>
        <tr>
            <td>
                <p><strong><?php _e( 'Allow a POST or GET &#34;redirect_to&#34; logout variable to take redirect precedence', 'custom-login-redirect' ); ?></strong></p>
            </td>
            <td>
                <select name="rul_allow_post_redirect_override_logout">
                    <option value="1"<?php if( $rul_settings['rul_allow_post_redirect_override_logout'] ) print ' selected="selected"'; ?>><?php _e( 'Yes', 'custom-login-redirect' ); ?></option>
                    <option value="0"<?php if( !$rul_settings['rul_allow_post_redirect_override_logout'] ) print ' selected="selected"'; ?>><?php _e( 'No', 'custom-login-redirect' ); ?></option>
                </select>
            </td>
        </tr>
        ...
        <tr>
            <td>
                <p><strong><?php print sprintf( __( 'Use external redirect file. Set this to &#34;Yes&#34; if you are using a plugin such as Gigya that bypasses the regular WordPress redirect process (and allows only one fixed redirect URL). Then, set the redirect URL in the other plugin to %s', 'custom-login-redirect' ), '<br />http://www.yoursite.com/wp-content/plugins/custom-login-redirect/wplogin_redirect_control.php' ); ?></strong></p>
            </td>
            <td>
                <select name="rul_use_redirect_controller">
                    <option value="1"<?php if( $rul_settings['rul_use_redirect_controller'] ) print ' selected="selected"'; ?>><?php _e( 'Yes', 'custom-login-redirect' ); ?></option>
                    <option value="0"<?php if( !$rul_settings['rul_use_redirect_controller'] ) print ' selected="selected"'; ?>><?php _e( 'No', 'custom-login-redirect' ); ?></option>
                </select>
            </td>
        </tr>
        <tr>
            <td>
                <p><strong><?php _e( 'Permission level required to edit redirect URLs', 'custom-login-redirect' ); ?></strong></p>
            </td>
            <td>
        ...
        <p class="submit"><input name="rul_settingssubmit" type="submit" value="<?php _e( 'Update', 'custom-login-redirect' ); ?>" /></p>
        </form>
    </div>
<?php
